...
export default async function ProductPage({ params }: Props) {
  ...
  return <main className="content-page product-page"><header className="product-top"><a className="mini-logo" href="/" aria-label="Volver al inicio">Wandra</a><a className="back-link" href="/productos">← Todos los productos</a></header><article className="product-detail"><div className="product-media"><ProductGallery images={product.images || [product.image]} alt={product.name} /></div><div className="product-copy"><p className="eyebrow">{product.category}</p><h1>{product.name}</h1><p className="product-description">{product.description}</p><div className="variant-list" aria-label="Presentaciones y precios">{product.variants.map((variant) => <div key={variant.sku}><span>{variant.label}</span><strong>{formatCOP(variant.price)}</strong></div>)}</div><p className="product-note">Precio desde {formatCOP(lowestPrice)}. La disponibilidad, ingredientes y condiciones de envío se confirmarán en el checkout.</p><a className="button button-dark" href="/#comprar">Consultar compra</a></div></article><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} /></main>;
}
